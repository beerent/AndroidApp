...
//TFTP Server and Client files thanks to Eddie Banner.
...
public class ServerTFTP extends Thread {
    ...
    try {
      ...
      if (buf[0] == 0 && buf[1] == 1) {  // it's a RRQ
        ...
        try {  // check to see if file exists
          ...
          SenderTFTP sender = new SenderTFTP(address, port, fileData, sss);
          sender.send();
        ...
        ReceiverTFTP receiver = new ReceiverTFTP(address, port, fileOutput, sss, block);
        receiver.receive();
  ...
  public ServerTFTP(){
	  try {
		DatagramSocket srv = new DatagramSocket(10018);
	    byte [] buf = new byte[516];
